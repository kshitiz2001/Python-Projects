...
kalam.left(90)
kalam.penup()
kalam.goto(-7 * kshitiz, 0)
kalam.pendown()

for a in range(-7 * kshitiz, -3 * kshitiz, 1):
    x = a / kshitiz
    rel = math.fabs(x)
    ...
    kalam.goto(a, y * kshitiz)

for a in range(-3 * kshitiz, -1 * kshitiz - 1, 1):
    x = a / kshitiz
    rel = math.fabs(x)
...
kalam.goto(-1 * kshitiz, 3 * kshitiz)
kalam.goto(int(-0.5 * kshitiz), int(2.2 * kshitiz))
kalam.goto(int(0.5 * kshitiz), int(2.2 * kshitiz))
kalam.goto(1 * kshitiz, 3 * kshitiz)
print("Batman Logo with Python Turtle")
for a in range(1 * kshitiz + 1, 3 * kshitiz + 1, 1):
    x = a / kshitiz
    rel = math.fabs(x)
    y = (2.71052 + 1.5 - 0.5 * rel - 1.35526 * math.sqrt(4 - (rel - 1) ** 2)) * math.sqrt(
        math.fabs(rel - 1) / (rel - 1))
...
for a in range(3 * kshitiz + 1, 7 * kshitiz + 1, 1):
    x = a / kshitiz
    rel = math.fabs(x)
    y = 1.5 * math.sqrt((-math.fabs(rel - 1)) * math.fabs(3 - rel) / ((rel - 1) * (3 - rel))) * (
                1 + math.fabs(rel - 3) / (rel - 3)) * math.sqrt(1 - (x / 7) ** 2) + (
